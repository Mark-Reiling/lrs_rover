...
bool create_executor (lrs_srvs_exec::TSTCreateExecutor::Request  &req,
  ...
  if (type == "start-collect-data") {
    execmap[os.str()] = new Exec::StartCollectData (req.ns, req.id);
    found = true;
  }

  if (type == "stop-collect-data") {
    execmap[os.str()] = new Exec::StopCollectData (req.ns, req.id);
    found = true;
  }
...
int main(int argc, char **argv) {
  ...
  services.push_back (n.advertiseService(prefix + "executor_lock_resources_check", executor_lock_resources_check));
